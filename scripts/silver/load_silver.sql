...
- Uses the 'INSERT INTO' command to load cleaned and tranformed data from bronze to silver tables.
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT
			cst_id, 
			cst_key,
			TRIM(cst_firstname) AS cst_firstname, -- Removing unwanted space in the firstname column
			TRIM(cst_lastname) AS cst_lastname, -- Removing unwanted space in the lastname column
			CASE
		...
		SELECT
			prd_id,
			REPLACE(SUBSTRING(TRIM(prd_key), 1, 5), '-', '_') AS cat_id, -- Extra category ID column
			SUBSTRING(TRIM(prd_key), 7, LEN(prd_key)) AS prd_key, -- Extra product key column
			prd_nm,
		...
		SELECT
			CASE
				WHEN CID LIKE 'NAS%' THEN SUBSTRING(CID, 4, LEN(CID)) -- Removing the unwanted 'NAS' prefix in the cid column
				ELSE CID
			END AS CID,
			CASE
				WHEN BDATE > GETDATE() THEN NULL
				ELSE BDATE
			END AS BDATE, -- Handling future birth dates
			CASE
				WHEN UPPER(TRIM(GEN)) IN ('F', 'FEMALE') THEN 'Female'
				WHEN UPPER(TRIM(GEN)) IN ('M', 'MALE') THEN 'Male'
				ELSE 'n/a'
			END AS GEN -- Mapping Gender code values to meaningful valies
		...
		SELECT
			...
			CASE
				...
				WHEN UPPER(TRIM(CNTRY)) IN ('USA', 'UINTED STATES', 'US') THEN 'United States'
